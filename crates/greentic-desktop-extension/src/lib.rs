...
pub fn built_in_extension(extension_id: &str) -> Option<ExtensionManifest> {
    match extension_id {
        ...
        "greentic.desktop.excel" => Some(ExtensionManifest {
            id: extension_id.to_owned(),
            name: "Excel Workbook Adapter".to_owned(),
            version: "1.0.0".to_owned(),
            runtime: ExtensionRuntime::Native,
            command: None,
            args: Vec::new(),
            capabilities: vec![
                "excel.list_sheets".to_owned(),
                "excel.preview_sheet".to_owned(),
                "excel.read_range".to_owned(),
                "excel.detect_headers".to_owned(),
                "excel.search_cells".to_owned(),
                "excel.search_rows".to_owned(),
                "excel.validate_schema".to_owned(),
                "excel.update_cells".to_owned(),
                "excel.append_rows".to_owned(),
                "excel.create_sheet".to_owned(),
                "excel.write_range".to_owned(),
                "excel.create_workbook".to_owned(),
                "excel.write_table".to_owned(),
                "excel.create_report".to_owned(),
            ],
            permissions: vec!["filesystem.read".to_owned(), "filesystem.write".to_owned()],
            signed: true,
        }),
