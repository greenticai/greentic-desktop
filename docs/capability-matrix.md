...
| --- | --- | --- | --- | --- | --- |
| Browser and web apps | `greentic.desktop.playwright` | Beta | Playwright/web adapter installed; Greentic-owned browser context for recording. | Web runner replay through the GUI-managed MCP path against a local fixture, with output and evidence files persisted. | It records the controlled browser context Greentic opens, not arbitrary existing browser tabs. |
