...
- **Path:** `crates/greentic-desktop-linux`
  - **Role:** Linux desktop adapters (X11 and Wayland) with a native AT-SPI backend.
  - **Key functionality:** `accessibility/` holds a toolkit-neutral model: an `AccessibleBackend` trait, `TreeSnapshot` capture with depth/node limits, pure locator matching (role mapping, emoji/`:`/`*`-tolerant names, DOM `id` attribute ranking with id-only fallback), label→next-sibling reads, digit-aware option matching, and an `AccessibilityExecutor` for find/assert/click/type/read/tree dump with polling. `choice.rs` selects combo-box options inline or through a toolkit popup window (WebKitGTK `<select>`) and verifies the selection; typing uses EditableText or, for Text-only controls, focus + select + AT-SPI keyboard synthesis with read-back verification. `live.rs` implements the backend over the `atspi` proxies on a `zbus` connection (uncached proxies, raw `(so)` child references, `ChildCount`/`GetChildAtIndex`, `GetRole` before `GetRoleName`). `atspi_session.rs` holds the lazily connected bus and the window scope (clicks/typing refuse to run unscoped); `launch.rs` resolves executables and `.desktop` entries for `linux.open_app`. X11 capabilities: `linux.open_app`, `linux.find_window`, `linux.read_window_tree`, `linux.find_element`, `linux.click_element`, `linux.type_text`, `linux.press_shortcut`, `linux.read_text`, `linux.assert_visible`, `linux.screenshot`, `linux.activate_window`, `linux.close_window`; Wayland adds the AT-SPI element capabilities to `linux.wayland.*` but keeps global input, window control and keyboard synthesis (unless `GREENTIC_LINUX_ATSPI_KEYBOARD=1`) unsupported. `wmctrl`/`xdotool` remain only for window-manager control, shortcuts, untargeted typing and region clicks. Unit tests run against an in-memory fixture and never open the live bus.
